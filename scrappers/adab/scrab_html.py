...
# Constants
POEM_URL = "https://www.adab.com/post/view_post/{}"
POET_URL = "https://poetsgate.com/poet.php?pt={}"
SAVE_DIR = "/path/to/data/raw/adab"
...
def download_items(start_id, end_id, url_template, save_dir, item_type, failed_file, max_workers=10):
    ...
    pending_ids = [
        item_id for item_id in range(start_id, end_id)
        if not os.path.exists(os.path.join(save_dir, f"{item_type}_{item_id}.html"))
    ]

    logging.info(f"Found {len(pending_ids)} {item_type}s to download (out of {end_id - start_id})")
...
if __name__ == "__main__":
    ...
    download_items(0, 95000, POEM_URL, POEMS_DIR, "poem", FAILED_POEMS_FILE, max_workers=24)
